...
int crete_concolicTest_tofile(const crete::TestCasePatchTraceTag_ty& tcp_tt,
        ...
        const unsigned id, const bool from_captured_br)
{
    ...
    ctc.set_tt_id(g_qemu_rt_Info->get_tt_node_pc(tcp_tt.first), tcp_tt.second);
...
#endif
{
  ...
  if (!NoOutput) {
    ...
    if(concolic_success)
    {
        ...
        if (!crete_concolicTest_tofile(tcp_tt, tcp_elems, g_qemu_rt_Info->get_base_tc_issue_index(), id, is_captured_br))
        {
